...
import type { PackageList } from "./packageColumns";
...
const PACKAGE_COLUMN_ORDERING_KEY = "cash-sales.package.bs.table.ordering";
const PACKAGE_COLUMN_SIZING_KEY = "cash-sales.package.bs.table.sizing";
const COLUMN_VISIBILITY_KEY = "cash-sales.package.bs.table.columns";
...
export default function PackagePopover({
  ...
  const [packageColumnOrder, setPackageColumnOrder] = useLocalStorage<ColumnOrderState>(
    PACKAGE_COLUMN_ORDERING_KEY,
    initialColumnOrder
  );
  const [packageColumnSizing, setPackageColumnSizing] = useLocalStorage<ColumnSizingState>(
    PACKAGE_COLUMN_SIZING_KEY,
    {}
  );

  const packageDefaultColumns: Array<keyof PackageList> = [
    "packageCode",
    "packageName",
    "description",
  ];

  const packageDefaultColumnVisibility: VisibilityState = getExcludedColumns(
    columns || [],
    packageDefaultColumns
  );
  ...
  >(() => {
    ...
    return packageDefaultColumnVisibility;
  });
  ...
  const table = useReactTable({
    ...
    state: {
      ...
      columnSizing: packageColumnSizing,
    },
    onSortingChange: setSorting,
    onColumnVisibilityChange: setColumnVisibility,
    onColumnOrderChange: setPackageColumnOrder,
    onColumnSizingChange: setPackageColumnSizing,
  ...
  const savePackageServices = async (prop: any) => {
    try {
      ...
      const response = await fetch(
        `${ORIGIN}/cash_sales/api/cash-sales/load-package-has-service${id && slug !== "clone" ? `?id=${id}` : ""}`,
        {
          method: "POST",
          headers,
          body: formData,
  ...
  const onDoubleClick = (row: Row<PackageList>) => {
            ...
            <ScrollArea className="h-[100cqh] bg-erp-gray-3">
              ...
              <div className="hidden md:block">
                ...
                <DndContext
                  ...
                  onDragEnd={(event) => handleDndDragEnd(event, setPackageColumnOrder)}
                  sensors={sensors}
                >
...
// Column Drag and Drop and Resize (Table Body)
type TableBodyProps = {
  table: TableType<PackageList>;
  onRowSelection: (row: Row<PackageList>) => (event: MouseEvent<HTMLTableRowElement>) => void;
  columns: ColumnDef<PackageList>[];
};
