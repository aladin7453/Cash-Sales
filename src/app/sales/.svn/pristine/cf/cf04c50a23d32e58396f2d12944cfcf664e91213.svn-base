...
import CashSalesDetailsDataTableToolbarOld from "./DataTableToolbarOld";
...
export default function CashSalesDetailsDataTableOld({
  columns,
  data,
  ...
  // Watch for changes in form's additional discount and sync with local state
  useEffect(() => {
  ...
  function setTempRowCashSalesDetailsListData(data, overwrite) {
    if (overwrite) {
      ...
      // Make sure we're handling both array and non-array states correctly
      setTempRowCashSalesDetailsList((prevList) => {
        // Handle case where prevList might not be an array (e.g., null or undefined)
  ...
  // Use tempRowCashSalesDetailsList if it exists, otherwise use data
  const tableData =
  ...
  const fetchItemData = async (itemType: string) => {
    try {
      const response = await fetch(
        `${ORIGIN}/universal/get-all-item-details?itemType=${itemType}`,
        {
          headers,
        },
      );
  ...
  const calculateNetTotal = (convertToLocal: boolean) => {
    ...
    if (convertToLocal) {
      total *= currentCurrencyRate; // Accurate Currency Rate Update
    }
  ...
  const numberInputCell = ({ row, column: { id } }, allowDecimal: boolean) => {
    ...
    // Check if discount fields should be disabled for cash sales
    // const disableDiscountFields =
    //   userHasRuleData?.userHasRule.includes("disable-cash-sales-discount-update") || false;

    const disableDiscountFields =
      userHasRuleData?.userHasRule?.includes("enable-cash-sales-discount-update") ?? false;
    ...
    return (
      <NumberInputCellComponent
        ...
        setTempRowDetailsList={setTempRowCashSalesDetailsList}
        userHasRuleData={userHasRuleData}
  ...
  const updateItem = async (prop) => {
    ...
    try {
      ...
      const rowTotalDiscount2 = table.getRowModel().rows.reduce((sum, row) => {
        return sum + (row.original.discountAmount2 ? parseFloat(row.original.discountAmount2) : 0);
      }, 0);

      const rowTotalDiscount3 = table.getRowModel().rows.reduce((sum, row) => {
      ...
      const totalTax = table.getRowModel().rows.reduce((sum, row) => {
        return sum + (row.original.taxAmount ? parseFloat(row.original.taxAmount) : 0);
      }, 0);

      const totalFeeAmount = 0; // Set as needed or calculate from rows
      ...
      // Create cash sales object with totals
      const cashSalesData = {
        ...
        outstandingAmount: roundTotalPayableAfterPrepayment.toFixed(2), // Set outstanding amount to subtotal tax
      ...
      // Get current cash sales details
      ...
      if (prop.isUpdate) {
        ...
        const newItems = selectedRowModel.map((row, index) => {
          ...
          return {
            ...
            itemGroup:
              prop.itemType === "service" ? item.serviceGroupCode : item.stockGroupCodeCode,
            itemCategory:
              prop.itemType === "service" ? item.serviceCategoryCode : item.stockCategoryCodeCode,
            tariff: null,
      ...
      // Combine cashSales and cashSalesHasDetails into single data object
      const combinedData = {
  ...
  // Handle tax inclusive, master service and show in PDF checkbox changes
  ...
  const handleTaxInclusiveCheckedChanged = (rowIndex, isChecked, row) => {
    ...
    // Update the tempRowCashSalesDetailsList
    setTempRowCashSalesDetailsList((prevList) => {
      return prevList.map((item, idx) => {
        // Find the matching row by UUID or index if no UUID
  ...
  const handleMobileFieldChange = (rowId: string, fieldId: string, value: any) => {
    ...
    // Update tempRowCashSalesDetailsList
    setTempRowCashSalesDetailsList((prevList) =>
      prevList.map((item, idx) => {
  ...
  useEffect(() => {
    if (isPaidAmountEmpty && !isPaidAmountManuallyUpdated) {
      const netTotal = calculateNetTotal(false); // Calculate Net Total (RM)

      form.setValue("paidAmount", netTotal); // Update paidAmount in the form
    }
  ...
  const calculateRowValues = (rowData: any, rowIndex: number) => {
    ...
    const quantity2 = rowData.quantity2 ? Number(rowData.quantity2) : rowData.type == "Stock" ? 1 : rowData.quantity2 === 0 ? 0 : 1;
    const quantity3 = rowData.quantity3 ? Number(rowData.quantity3) : rowData.type == "Stock" ? 1 : rowData.quantity3 === 0 ? 0 : 1;
  ...
  const handleCloneRows = () => {
    ...
    if (multipleCloneRowCopy.length > 0) {
      setTempRowCashSalesDetailsList((prev) => {
        const newList = [...prev];
      ...
      const cloneRowCopy = JSON.parse(
        JSON.stringify(tempRowCashSalesDetailsList.find((item, index) => index === cloneIndex)),
      );
      setTempRowCashSalesDetailsList((prev) => {
        const newList = [...prev];
        cloneRowCopy.UUID = "";
  ...
  return (
    <Card collapsible defaultCollapsed={false} className="h-auto p-0 shadow-md">
      <CardHeader>
        <CardTitle className="text-lg font-bold">Charges Details</CardTitle>
      </CardHeader>
      <CardContent className="px-2 py-0">
        <div className="h-[98cqh] p-2 md:h-[58cqh]">
          <div className="flex h-[95cqh] flex-col rounded-md bg-erp-blue-3 @container-[size] md:h-[55cqh]">
            <CashSalesDetailsDataTableToolbarOld
              table={table}
              onAdd={handleOnAdd}
              ...
              <div className="hidden md:block">
                ...
                >
                  ...
                  <DragDropDataTable items={items} disabled={false} hasParentDndContext={true}>
                    ...
                    <Table
                      className="table-fixed border-separate border-spacing-[3px] overflow-visible whitespace-nowrap border bg-erp-gray-1"
            ...
            <div className="hidden flex-grow flex-row bg-white md:flex">
              <div className="flex flex-grow flex-row border-2 border-solid border-erp-blue-18">
                ...
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Quantity:</p>
                  <Input
                    className="h-3 w-28 text-right text-sm"
                    readOnly
                    value={table.getRowModel().rows ? calculateTotal("quantity") : 0}
                ...
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Total Amount:</p>
                  <Input
                    className="h-3 w-28 text-right text-sm"
                    readOnly
                ...
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Total Discount:</p>
                  <Input
                    className="h-3 w-28 text-right text-sm"
                    readOnly
                    value={parseFloat(calculateTotalDiscount()).toFixed(2)}
                  />
                </div>

                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Subtotal:</p>
                  <Input
                    className="h-3 w-28 text-right text-sm"
                    readOnly
                    value={table.getRowModel().rows ? calculateTotal("subTotal") : 0}
                ...
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Tax Amount:</p>
                  <Input
                    className="h-3 w-28 text-right text-sm"
                    readOnly
                    value={table.getRowModel().rows ? calculateTotal("taxAmount") : 0}
              ...
              <div className="flex flex-none flex-row border-2 border-solid border-erp-blue-18">
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Local Subtotal (Tax):</p>
                  <Input
                    className="localNetTotal h-3 w-28 text-right text-sm"
                    readOnly
                    value={table.getRowModel().rows ? calculateNetTotal(true) : 0}
                  />
                </div>
                ...
                <div className="flex flex-col justify-start p-[0.5rem]">
                  <p className="text-left text-sm">Subtotal (Tax) - Rounding:</p>
                  <Input
                    className="netTotal h-3 w-28 text-right text-sm"
                    readOnly
                    value={table.getRowModel().rows ? calculateNetTotal(false) : 0}
                  />
                </div>
...
type TableBodyProps = {
  ...
  numberInputCell: (cell: any, allowDecimal: number, allowNegative?: boolean) => JSX.Element;
                  ...
                  return (
                    ...
                    >
                      {numberInputCell(
                        cell,
                        true,
                        userHasRuleData?.userHasRule.includes("update-cash-sales-below-0"),
                      )}
                    </TableCell>
                  );
                } else if (
                  ...
                  return (
                    ...
                    >
                      <p className="text-right">{`${(
                        ...
                        (form.getValues("currencyRate")
                          ? Number(form.getValues("currencyRate"))
                          : 1)
                      ).toFixed(2)}`}</p>
                    </TableCell>
                  );
