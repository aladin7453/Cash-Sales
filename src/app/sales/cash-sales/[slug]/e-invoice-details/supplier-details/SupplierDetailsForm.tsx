"use client";

import React, { useEffect, useRef, useState } from "react";
import { FaMagnifyingGlass } from "react-icons/fa6";

import DescriptionCellTooltip from "@/components/data-table/DescriptionCellTooltip";
import DropdownTable from "@/components/data-table/DropdownTable";
import { Checkbox } from "@/components/ui/checkbox";
import { Input } from "@/components/ui/input";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Textarea } from "@/components/ui/textarea";
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from "@/components/ui/form";
import {
  Card,
  CardContent,
  CardHeader,
  CardTitle
} from "@/components/ui/card";
import {
  TooltipProvider,
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import {
  cacheCurrentCompany,
  getCachedCurrentCompany,
} from "@/components/offlineDB";

import { getAuthHeaders, ORIGIN } from "@/lib/constants";

type Params = {
  form: any;
  supplierCode: string;
  defaultCurrentCompany: any;
};

export function SupplierDetailsForm({ form, supplierCode, defaultCurrentCompany }: Params) {
  const headers = getAuthHeaders();

  const [currentCompanyUUID, setCurrentCompanyUUID] = useState<string | null>(null);

  useEffect(() => {
    const currentCompany = JSON.parse(localStorage.getItem("currentCompany") || "null");

    if (currentCompany && currentCompany.UUID) {
      setCurrentCompanyUUID(currentCompany.UUID);
    } else {
      setCurrentCompanyUUID(null);
    }
  }, []);

  const inputRef = useRef<HTMLInputElement>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const handleClickOutside = (event: MouseEvent) => {
    if (
      dropdownRef.current &&
      !dropdownRef.current.contains(event.target as Node) &&
      inputRef.current !== event.target
    ) {
      setShowTableMSICCode(false);
    }
  };

  useEffect(() => {
    document.body.addEventListener("click", handleClickOutside);

    return () => {
      document.body.removeEventListener("click", handleClickOutside);
    };
  }, []);

  // MSIC Code Dropdown
  const [showTableMSICCode, setShowTableMSICCode] = useState(false);
  const [MSICCodeDropdownTableData, setMSICCodeDropdownTableData] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");

  const toggleTableMSICCode = async () => {
    const formData = new FormData();
    const MSICCode = "MISCCode";

    formData.append("table[]", MSICCode);

    if (!showTableMSICCode) {
      try {
        const response = await fetch(`${ORIGIN}/universal/get-all-drop-down-table-data`, {
          method: "POST",
          headers,
          body: formData,
        });

        if (!response.ok) {
          throw new Error("Failed to fetch agent drop-down data");
        }

        const MSICCodeData = await response.json();

        setMSICCodeDropdownTableData(MSICCodeData.rows);
        setShowTableMSICCode(true);
      } catch (error) {
        console.error("Error fetching MSICCode drop-down data:", error);
      }
    } else {
      setShowTableMSICCode(false);
    }
  };

  // Auto-fetch current company UUID and fill in fields on mount
  useEffect(() => {
    const currentCompany = JSON.parse(
      localStorage.getItem("currentCompany") || "null",
    );

    if (currentCompany?.UUID) {
      setCurrentCompanyUUID(currentCompany.UUID);

      const fetchAndFill = async () => {
        if (!navigator.onLine) {
          const cached = await getCachedCurrentCompany();
          if (!cached) return;
          form.setValue("supplierName", cached.company);
          form.setValue("supplierTIN", cached.TIN);
          form.setValue("supplierBRN", cached.BRN);
          form.setValue("supplierSSTNo", cached.SSTNo);
          form.setValue("supplierTTXNo", cached.TTxNo);
          form.setValue("supplierMSICCode", cached.MSIC);
          form.setValue("MISCCodeCode", cached.MSICCode);
          setSearchTerm(cached.MSICCode);
          form.setValue("supplierBusinessNature", cached.businessNature);
          form.setValue("supplierEmail", cached.email);
          form.setValue("supplierPhoneNo", cached.phoneNo);
          form.setValue("supplierAddress", cached.address);
          return;
        }
        try {
          const authHeaders = getAuthHeaders();
          const res = await fetch(
            `${ORIGIN}/universal/get-current-company?id=${currentCompany.UUID}`,
            { headers: authHeaders },
          );

          if (!res.ok) throw new Error("Failed to fetch current company");

          const data = await res.json();
          const companyData = data.data;
          await cacheCurrentCompany(companyData);

          form.setValue("supplierName", companyData.company);
          form.setValue("supplierTIN", companyData.TIN);
          form.setValue("supplierBRN", companyData.BRN);
          form.setValue("supplierSSTNo", companyData.SSTNo);
          form.setValue("supplierTTXNo", companyData.TTxNo);
          form.setValue("supplierMSICCode", companyData.MSIC);
          form.setValue("MISCCodeCode", companyData.MSICCode);
          setSearchTerm(companyData.MSICCode);
          form.setValue("supplierBusinessNature", companyData.businessNature);
          form.setValue("supplierEmail", companyData.email);
          form.setValue("supplierPhoneNo", companyData.phoneNo);
          form.setValue("supplierAddress", companyData.address);
        } catch (error) {
          console.error("Error fetching current company data:", error);
        }
      };

      fetchAndFill();
    }
  }, []);

  // Fallback
  useEffect(() => {
    if (defaultCurrentCompany) {
      const companyData = defaultCurrentCompany.data;

      form.setValue("supplierName", companyData.company);
      form.setValue("supplierTIN", companyData.TIN);
      form.setValue("supplierBRN", companyData.BRN);
      form.setValue("supplierSSTNo", companyData.SSTNo);
      form.setValue("supplierTTXNo", companyData.TTxNo);
      form.setValue("supplierMSICCode", companyData.MSIC);
      form.setValue("MISCCodeCode", companyData.MSICCode);
      setSearchTerm(companyData.MSICCode);
      form.setValue("supplierBusinessNature", companyData.businessNature);
      form.setValue("supplierEmail", companyData.email);
      form.setValue("supplierPhoneNo", companyData.phoneNo);
      form.setValue("supplierAddress", companyData.address);
    }
  }, [defaultCurrentCompany]);

  return (
    <Card className="w-full !shadow-none border-none">
      <div className="w-full flex justify-between items-center px-0.5 pt-1.5">
        <CardTitle className="text-sm font-bold">
          Me
        </CardTitle>
      </div>
      <CardContent className="p-0">
        <Form {...form}>
          <form>
            <div className="flex flex-col gap-y-2.5 pt-1.5">
              {/* First Row */}
              <div className="grid grid-cols-3 items-center gap-x-2">
                {/* Supplier UUID */}
                <FormField
                  control={form.control}
                  name="supplier"
                  render={({ field }) => (
                    <FormItem className="hidden">
                      <FormControl>
                        <Input className="h-7.5" {...field} readOnly />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* Supplier Name */}
                <FormField
                  control={form.control}
                  name="supplierName"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                moduleName="salesInvoice"
                                fieldName="supplierName"
                                columnName="Supplier Name"
                                placeholder="Supplier Name"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier Name
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* Supplier TIN */}
                <FormField
                  control={form.control}
                  name="supplierTIN"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                moduleName="salesInvoice"
                                fieldName="supplierTIN"
                                columnName="Supplier TIN"
                                placeholder="Supplier TIN"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier TIN
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* Supplier BRN/NRIC */}
                <FormField
                  control={form.control}
                  name="supplierBRN"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                placeholder="Supplier BRN/NRIC"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier BRN/NRIC
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              </div>

              {/* Second Row */}
              <div className="grid grid-cols-3 items-center gap-x-2">
                {/* Supplier SST No */}
                <FormField
                  control={form.control}
                  name="supplierSSTNo"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                placeholder="Supplier SST No"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier SST No
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* TTX No */}
                <FormField
                  control={form.control}
                  name="supplierTTXNo"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                placeholder="TTx No"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              TTx No
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* MSIC UUID (hidden) */}
                <FormField
                  control={form.control}
                  name="supplierMSICCode"
                  render={({ field }) => (
                    <FormItem className="hidden">
                      <FormControl>
                        <Input className="h-7.5" {...field} readOnly={true} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* MSIC Code */}
                <FormField
                  control={form.control}
                  name="MISCCodeCode"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl className="relative">
                        <div className="relative">
                          <TooltipProvider delayDuration={1000}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Input
                                  className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                  {...field}
                                  onClick={toggleTableMSICCode}
                                  ref={inputRef}
                                  value={searchTerm}
                                  onChange={(e) => {
                                    setSearchTerm(e.target.value);
                                    setShowTableMSICCode(true);
                                  }}
                                  autoComplete="off"
                                  placeholder="MSIC"
                                  readOnly
                                />
                              </TooltipTrigger>
                              <TooltipContent side="top" align="center">
                                MISC
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </div>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              </div>

              {/* Third Row */}
              <div className="grid grid-cols-3 items-center gap-x-2">
                {/* Business Nature */}
                <FormField
                  control={form.control}
                  name="supplierBusinessNature"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                placeholder="Business Nature"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Business Nature
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* Supplier Email */}
                <FormField
                  control={form.control}
                  name="supplierEmail"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                moduleName="salesInvoice"
                                fieldName="supplierEmail"
                                columnName="Supplier Email"
                                placeholder="Supplier Email"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier Email
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />

                {/* Supplier Phone */}
                <FormField
                  control={form.control}
                  name="supplierPhoneNo"
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Input
                                className="h-7 rounded-md bg-erp-gray-1 border-gray-200 px-2 text-[11px] text-gray-black focus-visible:ring-0 focus-visible:outline-none"
                                {...field}
                                placeholder="Supplier Phone"
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier Phone
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              </div>

              {/* Supplier Address */}
              <div className="flex-1">
                <FormField
                  control={form.control}
                  name="supplierAddress"
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormControl>
                        <TooltipProvider delayDuration={1000}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Textarea
                                className="h-[85px] rounded-md bg-erp-gray-1 border-gray-200 px-2 resize-none text-[11px] text-black focus-visible:ring-0 focus-visible:outline-none"
                                placeholder="Supplier Address"
                                {...field}
                                readOnly
                              />
                            </TooltipTrigger>
                            <TooltipContent side="top" align="center">
                              Supplier Address
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              </div>
            </div>
          </form>
        </Form>
      </CardContent>
    </Card>
  );
}