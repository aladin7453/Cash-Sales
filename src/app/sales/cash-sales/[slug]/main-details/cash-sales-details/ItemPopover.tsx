...
type Props = {
  ...
  setTempRowCashSalesDetailsListData: (data: any, overwrite: boolean) => void;
...
const SERVICE_COLUMN_ORDERING_KEY = "cash-sales.service.bs.table.ordering";
const SERVICE_COLUMN_SIZING_KEY = "cash-sales.service.bs.table.sizing";
const STOCK_COLUMN_ORDERING_KEY = "cash-sales.stock.bs.table.ordering";
const STOCK_COLUMN_SIZING_KEY = "cash-sales.stock.bs.table.sizing";
const COLUMN_VISIBILITY_KEY = "cash-sales.item.bs.table.columns";
...
export default function ItemPopover({
  ...
  const stockDefaultColumns: Array<keyof ItemList> = ["stockCode", "stock", "description"];
  ...
  const setCurrentColumnOrder =
    itemType === "service" ? setServiceColumnOrder : setStockColumnOrder;
  const setCurrentColumnSizing =
    itemType === "service" ? setServiceColumnSizing : setStockColumnSizing;
    ...
    selectedRows.forEach((row, index) => {
      ...
      if (itemType === "service") {
        item = {
          ...
          itemCode: row.original["serviceCode"],
          itemName: row.original["serviceName"],
          type: "Service",
          ...
          itemGroup: row.original["serviceGroupCode"],
          itemGroupCode: row.original["serviceGroupCodeCode"],
          itemCategory: row.original["serviceCategoryCode"],
          itemCategoryCode: row.original["serviceCategoryCodeCode"],

          serviceGroupCodeCode: row.original["serviceGroupCodeCode"],
          serviceGroupName: row.original["serviceGroupName"],
          serviceCategoryCodeCode: row.original["serviceCategoryCodeCode"],
          serviceCategoryName: row.original["serviceCategoryName"],
          ...
          quantity: row.original.UOMDropdown && row.original.UOMDropdown.length > 0 && row.original.UOMDropdown[0].UOM ? 1 : "",
          quantity2: row.original.UOMDropdown2 && row.original.UOMDropdown2.length > 0 && row.original.UOMDropdown2[0].UOM2 ? 1 : "",
          quantity3: row.original.UOMDropdown3 && row.original.UOMDropdown3.length > 0 && row.original.UOMDropdown3[0].UOM3 ? 1 : "",
          UOM: row.original["defaultUOMSales"],
          itemUOM: row.original["defaultUOMSalesUOM"] || "",
          UOM2: null,
          UOM3: null,
          price: row.original["refPrice"] || 0,
          ...
          UOMDropdown2: row.original.UOMDropdown2 || [],
          UOMDropdown3: row.original.UOMDropdown3 || [],
        };
      } else {
        item = {
          UUID: "",
          ...
          stockGroupCodeCode: row.original["stockGroupCodeCode"],
          stockGroupName: row.original["stockGroupName"],
          stockCategoryCodeCode: row.original["stockCategoryCodeCode"],
          stockCategoryName: row.original["stockCategoryName"],
          ...
          discountRate: 0,
          discountAmount: 0,
          itemSerialNo: row.original["serialNo"],
          tax: null,
          deliveryDate: "",
      ...
      setTempRowCashSalesDetailsListData(item, false);
    });
    //}
            ...
            <div className="flex h-[70cqh] flex-col bg-erp-blue-3 @container-[size]">
              ...
              <ScrollArea className="h-[100cqh] bg-erp-gray-3">
                ...
                <MobilePopoverDataTable
                  table={table}
                  headerFields={[
                    itemType === "service" ? "serviceCode" : "stockCode",
                    itemType === "service" ? "serviceName" : "stock",
                  ]}
                  initialVisibleFields={4}
                  excludeFromGrid={["serviceCode", "stockCode"]}
                  showSelection={true}
