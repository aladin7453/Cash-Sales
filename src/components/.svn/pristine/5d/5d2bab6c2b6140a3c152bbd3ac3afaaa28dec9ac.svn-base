...
const PreviewPDF = ({
  ...
  const getRowValue = (item) => {
    if (showService2SummaryIDRemark1) {
      return Number(item.parentPrice || 0);
    } else {
      return Number(item.remark2 || 0);
    }
  };

  const getMonthKeyFromDateFormat = (dateStr) => {
  ...
  const groupItemsByMonth = (items, includeBF = true) => {
    return items.reduce((acc, item) => {
      const monthKey = getMonthKeyFromDateFormat(item.dateFormat);
      ...
      acc[monthKey].total += getRowValue(item);
  ...
  return (
    <Document title={previewData.docNo || ""}>
      {pages.map((previewDoc, index) => {
        ...
        const safeItems = Array.isArray(previewData)
        ...
        const sortedItems = [...safeItems].sort((a, b) => {
          if (!a.dateFormat) return 1;
          if (!b.dateFormat) return -1;

          return new Date(a.dateFormat) - new Date(b.dateFormat);
        });
        const groupedByMonth = groupItemsByMonth(sortedItems, !showService2SummaryIDRemark1);

        const monthEntries = Object.entries(groupedByMonth);
        const includeBF = !showService2SummaryIDRemark1;
        const monthsWithBF = monthEntries.reduce((acc, [monthKey, data], index) => {
          const previousMonth = acc[index - 1];

          const bfTotal = includeBF && previousMonth ? previousMonth.total : 0;
          const currentMonthTotal = data.total;
          const cumulativeTotal = includeBF ? bfTotal + currentMonthTotal : currentMonthTotal;

          acc.push({
            monthKey,
            items: data.items,
            bfTotal: includeBF ? bfTotal : 0,
        ...
        monthsWithBF.forEach(({ items, bfTotal }, monthIndex) => {
          if (bfTotal > 0) {
            rows.push({
              type: "bf",
              value: bfTotal,
              label: `Page ${monthIndex} Total b/f`,
            });
          }
        ...
        const ROWS_PER_PAGE = 34; // tweak if needed
        ...
        monthsWithBF.forEach(({ items, bfTotal }, monthIndex) => {
          ...
          // paginate THIS month only
          for (let i = 0; i < monthRows.length; i += ROWS_PER_PAGE) {
            paginatedPages.push(monthRows.slice(i, i + ROWS_PER_PAGE));
          }
        });
        ...
        const totalPagesForDoc = paginatedPages.length;

        const grandTotal = rows.reduce((sum, row) => {
          if (row.type === "item") {
            return sum + getRowValue(row.data);
          }
          return sum;
        }, 0);
        ...
        return (
          <>
            {paginatedPages.map((pageRows, pageIndex) => {
              return (
                <Page size="A4" key={pageIndex} style={[styles.page, { paddingBottom: 160 }]}>
                  <>
                    ...
                    <View style={[styles.headerRow, { height: 120 }]} fixed>
                      ...
                      <View style={[styles.headerLeft]}>
                        ...
                        <Text style={styles.headerText}>
                          {currentCompanyData.phoneNo
                            ? `Phone No.: ${currentCompanyData.phoneNo}`
                            : ""}
                        </Text>
                      ...
                      <View style={styles.headerRight}>
                        <View>
                          {Qr && <Image src={Qr} style={styles.qrCodeImage} alt="QR Code" />}
                        </View>
                        <View>
                          {Qr && (
                    ...
                    >
                      ...
                      >
                        {isNotMoreThanFiveLines ? (
                          <>
                            <View
                              style={[styles.verticalLine2, { right: "50%", height: 71 }]}
                              fixed
                            />
                          </>
                        ) : null}
                        ...
                        >
                          ...
                          >
                            <Text>{`Page ${pageIndex + 1} of ${totalPagesForDoc}`}</Text>
                          </View>
                        </View>
                      </View>
                    </View>
                    ...
                    <View style={styles.contentContainer}>
                      ...
                      <View style={[styles.table]}>
                        {/* Header */}
                        {showService2SummaryIDRemark1 === true ? (
                          <>
                            <View style={[styles.tableRowHeader, styles.tableHeader]} fixed>
                              ...
                              />
                              {/* after Date */}
                              <View style={[styles.tableCellHeader, { flex: 0.75 }]}>
                                <Text>Date</Text>
                              </View>
                              <View style={[styles.tableCellHeader, { flex: 2.25 }]}>
                                <Text>Name</Text>
                              </View>
                            ...
                            {/* {bfTotal > 0 && (
                              <View style={styles.tableRow}>
                                <View
                                  style={[styles.tableCell, { flex: 0.75, textAlign: "center" }]}
                                ></View>

                                <View style={[styles.tableCell, { flex: 2.25, textAlign: "left" }]}>
                                  <Text style={styles.boldText}>
                                    {pageIndex > 0 ? `Page ${pageIndex} Total b/f` : ""}
                                  </Text>
                                </View>
                                ...
                                <View
                                  style={[styles.tableCell, { flex: 0.75, textAlign: "right" }]}
                                >
                                  <Text>{formatDecimal(bfTotal)}</Text>
                                </View>
                              </View>
                            )} */}

                            {pageRows.some((row) => row.type === "bf") && (
                              <>
                                {pageRows.map((row, index) => {
                                  if (row.type !== "bf") return null;

                                  return (
                                    <View style={styles.tableRow} key={`bf-${index}`}>
                                      <View
                                        style={[
                                          styles.tableCell,
                                      ...
                                      <View
                                        style={[
                                          styles.tableCell,
                                          { flex: 2.25, textAlign: "left" },
                                        ]}
                                      >
                                        <Text style={styles.boldText}>{row.label}</Text>
                                      </View>
                                      <View style={[styles.tableCell, { flex: 0.5 }]} />
                                      <View style={[styles.tableCell, { flex: 0.75 }]} />
                                      <View
                                        style={[
                                          styles.tableCell,
                                          { flex: 0.75, textAlign: "right" },
                                        ]}
                                      >
                                        <Text>{formatDecimal(row.value)}</Text>
                                      </View>
                                    </View>
                                  );
                                })}
                            ...
                            {/* {items.map((item, index) => (
                              <>
                                <View style={styles.tableRow} key={index}>
                                  ...
                                  <View
                                    style={[styles.tableCell, { flex: 2.25, textAlign: "left" }]}
                                  >
                                  ...
                                  >
                                    <Text>{stripHtml(item["customerID"] || "")}</Text>
                                  </View>
                                  <View
                                    style={[styles.tableCell, { flex: 0.75, textAlign: "center" }]}
                                  >
                                    <Text>{insertLineBreak(item.remark1)}</Text>
                                  </View>
                                  <View
                                    style={[styles.tableCell, { flex: 0.75, textAlign: "right" }]}
                                  >
                          ...
                          <>
                            <View style={[styles.tableRowHeader, styles.tableHeader]} fixed>
                              ...
                              />
                              {/* after Date */}
                              <View style={[styles.tableCellHeader, { flex: 0.45 }]}>
                                <Text>Date</Text>
                              </View>
                              <View style={[styles.tableCellHeader, { flex: 1.8 }]}>
                                <Text>Name</Text>
                              </View>
                              <View style={[styles.tableCellHeader, { flex: 0.75 }]}>
                                <Text>Sick Leave (Day)</Text>
                              </View>
                            </View>
                            ...
                            {/* {items.map((item, index) => (
                              <>
                                <View style={styles.tableRow} key={index}>
                                  <View
                                    style={[styles.tableCell, { flex: 0.45, textAlign: "center" }]}
                                  >
                                    <Text>{item.dateFormat}</Text>
                                  </View>
                                  <View
                                    style={[styles.tableCell, { flex: 1.8, textAlign: "left" }]}
                                  >
                                    <Text>{stripHtml(item["customerName"] || "")}</Text>
                                  </View>
                                  <View
                                    style={[styles.tableCell, { flex: 0.75, textAlign: "center" }]}
                                  >
                                    <Text>{insertLineBreak(item.remark2)}</Text>
                                  </View>
                                </View>
                              </>
                            ))} */}
                            ...
                            {pageRows.map((row, index) => {
                              if (row.type === "bf") {
                                return (
                                  <View style={styles.tableRow} key={`bf-${index}`}>
                                    <View style={[styles.tableCell, { flex: 0.45 }]} />
                    ...
                    <View style={styles.bottomSection} fixed>
                      {showService2SummaryIDRemark1 === true ? (
                        <>
                          <View style={styles.secondTypeSeparator} />
                          <View style={[styles.tableRowHeaderPayment, styles.tableHeaderPayment]}>
                            ...
                            <View
                              style={[
                                ...
                                { flex: 4.5, textAlign: "right" },
                              ]}
                            >
                              <Text>PAYMENT AMOUNT {previewDoc.currencySymbol}:</Text>
                            </View>
                            ...
                            >
                              ...
                              <Text style={{ textAlign: "right", fontSize: 7 }}>
                                {isNaN(Number(grandTotal)) ? "0.00" : grandTotal.toFixed(2)}
                              </Text>
                        ...
                        <>
                          ...
                          <View style={styles.botContainer}>
                            {/* Qty */}
                            <View style={[styles.leftBotColumn, { fontSize: 8 }]}>
                              <View style={[styles.remark, styles.boldText]}></View>
                            </View>

                            <View style={[styles.rightBotColumn, { marginTop: 20 }]}>
                              <>
                                {/* Current System User's Full Name */}
                                <View style={styles.systemUserSection}>
                                  <Text></Text>
