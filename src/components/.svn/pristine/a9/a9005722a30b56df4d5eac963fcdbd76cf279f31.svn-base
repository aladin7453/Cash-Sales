...
const PreviewPDF = ({
  ...
  const groupItemsByMonth = (items) => {
    return items.reduce((acc, item) => {
      ...
      acc[monthKey].total += Number(item.parentPrice || 0);
  ...
  return (
    <Document title={previewData.docNo || ""}>
      {pages.map((previewDoc, index) => {
        ...
        const groupedByMonth = groupItemsByMonth(sortedItems);
        ...
        const monthsWithBF = monthEntries.reduce((acc, [monthKey, data], index) => {
          ...
          const bfTotal = previousMonth ? previousMonth.total : 0;
          const currentMonthTotal = data.total;
          const cumulativeTotal = bfTotal + currentMonthTotal;
        ...
        return (
          <>
            {monthsWithBF.map(({ monthKey, items, total, bfTotal }) => (
              <Page size="A4" key={monthKey} style={[styles.page, { paddingBottom: 160 }]}>
                <>
                            ...
                            <View style={styles.tableRow}>
                              ...
                              <View style={[styles.tableCell, { flex: 2.25, textAlign: "left" }]}>
                                <Text
                                  ...
                                  render={({ pageNumber }) =>
                                    pageNumber > 1 ? `Page ${pageNumber - 1} Total b/f` : ""
                                  }
                                />
                              </View>
                              ...
                              <View style={[styles.tableCell, { flex: 0.75, textAlign: "right" }]}>
                          ...
                          {items.map((item, index) => (
                            <>
                              <View style={styles.tableRow} key={index}>
                                {/* <View style={{ position: "relative", width: 0, left: -12, textAlign: "right", fontSize: 7, }} > <Text>{index + 1}</Text> </View> */}
                                <View
                                  style={[styles.tableCell, { flex: 0.75, textAlign: "center" }]}
                                >
                          ...
                          {items.map((item, index) => (
                            <>
                              <View style={styles.tableRow} key={index}>
                                ...
                                <View style={[styles.tableCell, { flex: 1.8, textAlign: "left" }]}>
                                  <Text>{stripHtml(item["customerName"] || "")}</Text>
                                </View>
                                <View
                                  style={[styles.tableCell, { flex: 0.75, textAlign: "center" }]}
