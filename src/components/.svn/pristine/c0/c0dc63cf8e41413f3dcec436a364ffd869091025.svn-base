...
function renderHeader(
  ...
  return (
    <>
      ...
      <View>
        <Text style={[styles.sectionTitle, { textAlign: "center" }]}>{getReportTitle()}</Text>
      </View>
  ...
  return (
    <Document title={getDocumentTitle()}>
      <Page size="A4" orientation={pageOrientation} style={getPageStyles()} wrap>
        ...
        <View fixed style={getLogoStyles()}>
          {image64 ? (
            <Image
              src={{
                uri: image64,
        ...
        (stockReportData?.docType === "stockTransferLv1Date" ||
          ...
          data?.rows?.map((groupItem, groupIndex) => (
            <View key={groupIndex} wrap>
              {/* Group Title */}
              ...
              <View style={styles.table} wrap>
                ...
                {/* Table Rows */}
                {groupItem.rows.map((item, rowIndex) => (
        ...
        ) : type === "Stock Aging Report" && stockReportData?.docType === "stockAgingLSLv3" ? (
          data?.rows?.map((locationItem, locationIndex) =>
            locationItem.rows?.map((batchItem, batchIndex) =>
              batchItem.rows?.map((itemGroup, itemIndex) => (
                <View key={`${locationIndex}-${batchIndex}-${itemIndex}`} wrap>
                  {/* Title */}
                  <Text style={styles.groupTitle}>
                    Location: {locationItem.group || "Unknown"}, Batch:
                    {batchItem.group || "Unknown"}, Item Code: {itemGroup.group || "Unknown"}
                  </Text>
                  ...
                  <View style={styles.table} wrap>
                    ...
                    {/* Table Rows */}
                    {itemGroup.rows?.map((rowItem, rowIndex) => (
                      <View key={rowItem.UUID} style={styles.tableRow} wrap={false}>
                        {stockReportColumns?.map((column, colIndex) => {
                          if (hiddenColumns.includes(column.accessorKey)) return null;
                          ...
                          return (
                            ...
                            >
                              {column.header === "No"
                                ? rowIndex + 1
                                : insertZeroWidthSpace(rowItem[column.accessorKey], orientation)}
                            </Text>
                          );
                        })}
                      </View>
          ...
          (stockReportData?.docType === "stockAgingLSLv4StockGroup" ||
            stockReportData?.docType === "stockAgingLSLv4Category") ? (
          data?.rows?.map((locationItem, locationIndex) =>
            locationItem.rows?.map((batchItem, batchIndex) => {
              // Decide which middle-level items to render based on docType
              const middleItems =
                stockReportData?.docType === "stockAgingLSLv4StockGroup"
                  ? batchItem.rows // stockGroupItem
                  : stockReportData?.docType === "stockAgingLSLv4Category"
                    ? batchItem.rows // stockCategoryItem
                    : [];

              return middleItems?.map((middleItem, middleIndex) =>
                middleItem.rows?.map((itemGroup, itemIndex) => (
                  <View key={`${locationIndex}-${batchIndex}-${middleIndex}-${itemIndex}`} wrap>
                    {/* Title */}
                    <Text style={styles.groupTitle}>
                      Location: {locationItem.group || "Unknown"}, Batch:
                      {batchItem.group || "Unknown"}, Item Code: {itemGroup.group || "Unknown"},
                      {stockReportData?.docType === "stockAgingLSLv4StockGroup"
                        ? ` Stock Group: ${middleItem.group || "Unknown"}`
                        : ` Stock Category: ${middleItem.group || "Unknown"}`}
                    </Text>
          ...
          stockReportData?.docType === "stockSerialNumberConflictItemCode" &&
          stockReportData?.hasItemCode === true ? (
          data?.rows?.map((groupItem) => (
            <View key={groupItem.stockUUID} style={styles.table} wrap>
              <Text style={styles.groupTitle}>Item Code : {groupItem.stockCode}</Text>
              ...
              {/* Table Rows — ONLY this item’s rows */}
              {groupItem.rows?.map((item, rowIndex) => (
                <View key={item.UUID} style={styles.tableRow} wrap={false}>
                  {stockReportColumns?.map((column, colIndex) => {
                    if (hiddenColumns.includes(column.accessorKey)) return null;
          ...
          Object.values(
            Object.values(data?.rows || [])
              .flatMap((itemGroup) =>
                itemGroup.rows.flatMap((locationGroup) =>
                  locationGroup.rows.flatMap((categoryGroup) =>
                    (categoryGroup.rows || []).map((row) => ({
                      ...row,
                      __itemCode: itemGroup.group,
                      __location: locationGroup.group,
                      __category: categoryGroup.group,
                    })),
                  ),
                ),
              )
              .reduce((acc, row) => {
                const key = `${row.__itemCode}-${row.__location}-${row.__category}`;
                if (!acc[key]) {
                  acc[key] = {
                    itemCode: row.__itemCode,
            ...
            <View key={`${group.itemCode}-${group.location}-${group.category}`} wrap>
              {/* Group Title */}
              <Text style={styles.groupTitle}>
                ItemCode: {group.itemCode}, Location: {group.location}, Category: {group.category}
              </Text>
              ...
              <View style={styles.table} wrap>
                ...
                <View fixed style={[styles.tableRow, styles.tableHeader]}>
                  {stockReportColumns?.map((column, index) =>
                    hiddenColumns.includes(column.accessorKey) ? null : (
                      <Text
                        key={index}
                        style={
                          column.header === "No" ? getTableCellNoStyles() : getTableCellStyles()
          ...
          (stockReportData?.docType === "stockPhysicalWorksheetLv3" ||
            ...
            stockReportData?.docType === "stockPhysicalWorksheetLv3Batch") ? (
          Object.values(
            Object.values(data?.rows || {})
              .flatMap((itemGroup) =>
                asArray(itemGroup.rows).flatMap((locationGroup) =>
                  asArray(locationGroup.rows).flatMap((categoryGroup) =>
                    asArray(categoryGroup.rows).map((row) => ({
                      ...row,
                      __itemCode: itemGroup.group,
                      __location: locationGroup.group ?? row.itemLocationCode ?? "N/A",
                      __category: categoryGroup.group ?? row.stockCategoryName,
                    })),
                  ),
                ),
              )
          ...
          stockReportData?.docType === "stockQtyBalanceLv1MultiUOM" ? (
          <View style={styles2.table}>
            <View style={[styles2.tableRow, styles2.tableHeader]} fixed>
              <Text style={[styles2.cell, { width: COL.itemCode }]} />
              <Text style={[styles2.cell, { width: COL.desc }]} />
              <Text style={[styles2.cell, styles2.center, { width: "31%" }]}>MULTI</Text>
              <Text style={[styles2.cell, styles2.center, { width: "24%" }]}>BASE</Text>
            </View>

            <View style={[styles2.tableRow, styles2.tableHeader]} fixed>
              <Text style={[styles2.cell, { width: COL.itemCode }]}>Item Code</Text>
              <Text style={[styles2.cell, { width: COL.desc }]}>Description</Text>
              <Text style={[styles2.cell, { width: COL.qty }]}>Qty</Text>
              <Text style={[styles2.cell, { width: COL.uom }]}>UOM</Text>
              <Text style={[styles2.cell, { width: COL.loose }]}>Loose Qty</Text>
              <Text style={[styles2.cell, { width: COL.xRate }]}>x Rate</Text>
              <Text style={[styles2.cell, { width: COL.sqty }]}>SQTY</Text>
              <Text style={[styles2.cell, { width: COL.baseUom }]}>UOM</Text>
            </View>

            {data?.rows?.map((item: any, rowIndex: number) => (
              <View key={item.UUID || rowIndex} style={styles2.tableRow} wrap={false}>
                <Text style={[styles2.cell, { width: COL.itemCode }]}>{item.stockCode}</Text>
                <Text style={[styles2.cell, { width: COL.desc }]}>{item.description}</Text>
                <Text style={[styles2.cell, { width: COL.qty }]} />
                <Text style={[styles2.cell, { width: COL.uom }]} />
                <Text style={[styles2.cell, { width: COL.loose }]} />
                <Text style={[styles2.cell, { width: COL.xRate }]} />
                <Text style={[styles2.cell, { width: COL.sqty, textAlign: "right" }]}>
                  {item.balanceQuantityQuantity}
                </Text>
                <Text style={[styles2.cell, { width: COL.baseUom }]}>{item.baseUOM}</Text>
              </View>
            ))}
          </View>
        ) : type === "Stock Physical Worksheet" &&
          ...
          <View style={styles2.table}>
            <View style={[styles2.tableRow, styles2.tableHeader]} fixed>
              <Text style={[styles2.cell, { width: "45%" }]} />
              <Text style={[styles2.cell, styles2.center, { width: "55%" }]}>Location</Text>
            </View>

            <View style={[styles2.tableRow, styles2.tableHeader]} fixed>
              <Text style={[styles2.cell, { width: STATIC_COL.stockGroup }]}>Stock Group</Text>
              <Text style={[styles2.cell, { width: STATIC_COL.itemCode }]}>Item Code</Text>
              <Text style={[styles2.cell, { width: STATIC_COL.desc }]}>Description</Text>
              <Text style={[styles2.cell, { width: STATIC_COL.uom }]}>UOM</Text>
              {locations.map((loc) => (
                <Text key={loc} style={[styles2.cell, styles2.center, { width: locationColWidth }]}>
                  {breakLongText(loc)}
                </Text>
              ))}
              <Text style={[styles2.cell, styles2.center, { width: STATIC_COL.grandTotal }]}>
                Grand Total
              </Text>
            </View>

            {Object.values(data?.rows || {}).map((group: any, gIndex) => {
              const items = normalizeGroupItems(group);

              return items.map((item: any, iIndex: number) => {
                let grandTotal = 0;

                return (
                  <View key={`${gIndex}-${iIndex}`} style={styles2.tableRow} wrap={false}>
                    <Text style={[styles2.cell, { width: STATIC_COL.stockGroup }]}>
                      {item.stockGroupName}
                    </Text>
                    <Text style={[styles2.cell, { width: STATIC_COL.itemCode }]}>
                      {breakLongText(item.stockCode)}
                    </Text>
                    <Text style={[styles2.cell, { width: STATIC_COL.desc }]}>
                      {item.description}
                    </Text>
                    <Text style={[styles2.cell, styles2.center, { width: STATIC_COL.uom }]}>
                      {item.baseUOM}
                    </Text>
                    {locations.map((loc) => {
                      const qty = item.locationQty?.[loc] ?? 0;
                      grandTotal += qty;

                      return (
                        <Text
                          key={loc}
                          style={[styles2.cell, styles2.right, { width: locationColWidth }]}
                        >
                          {qty}
                        </Text>
                      );
                    })}
                    <Text style={[styles2.cell, styles2.right, { width: STATIC_COL.grandTotal }]}>
                      {grandTotal}
                    </Text>
                  </View>
                );
          ...
          stockReportData?.docType === "stockMonthEndBalanceLv3" ? (
          Object.values(
            Object.values(data?.rows || [])
              .flatMap((locationGroup) =>
                locationGroup.rows.flatMap((stockBatchGroup) =>
                  stockBatchGroup.rows.flatMap((itemGroup) =>
                    (itemGroup.rows || []).map((row) => ({
                      ...row,
                      __location: locationGroup.group,
                      __stockBatch: stockBatchGroup.group,
                      __itemCode: itemGroup.group,
                    })),
                  ),
                ),
              ...
              .reduce((acc, row) => {
                const key = `${row.__location}-${row.__stockBatch}-${row.__itemCode}`;

                if (!acc[key]) {
                  acc[key] = {
                    location: row.__location,
                    stockBatch: row.__stockBatch,
                    itemCode: row.__itemCode,
                    rows: [],
                  };
                }
            ...
            const groupedByItemCode = Object.values(
              group.rows.reduce((acc, row) => {
                ...
                acc[key].totalUnitCost += unitCost * qty;
                acc[key].rows.push(row);
            ...
            return (
              <View key={`${group.location}-${group.stockBatch}-${group.itemCode}`} wrap>
                {/* Group Title */}
                <Text style={styles.groupTitle}>
                  Location: {group.location}, Stock Batch: {group.stockBatch}, ItemCode:
                  {group.itemCode}
                </Text>
                ...
                <View style={styles.table} wrap>
                  ...
                  {groupedByItemCode.map((itemGroup) => (
                    <View key={itemGroup.stockCode}>
                      {itemGroup.rows.map((item, rowIndex) => {
                        return (
                          <View
                            key={`${item.UUID}-${rowIndex}`}
                          ...
                          >
                            {stockReportColumns?.map((column, colIndex) => {
                              ...
                              } else if (column.header === "Total Qty") {
                                value = rowIndex === 0 ? itemGroup.totalQty : "";
                              } else if (column.accessorKey === "stockCode") {
                                value = rowIndex === 0 ? itemGroup.stockCode : "";
                              } else if (column.accessorKey === "stockName") {
                                value = rowIndex === 0 ? itemGroup.stockName : "";
                              } else if (column.accessorKey === "utdCost") {
                                value = rowIndex === 0 ? itemGroup.totalUnitCost : "";
                              } else {
            ...
            stockReportData?.docType === "stockMonthEndBalanceLv4StockGroup") ||
          stockReportData?.docType === "stockMonthEndBalanceLv4StockCategory" ? (
          Object.values(
            Object.values(data?.rows || [])
              .flatMap((locationGroup) =>
                locationGroup.rows.flatMap((stockBatchGroup) =>
                  stockBatchGroup.rows.flatMap((itemGroup) =>
                    itemGroup.rows.flatMap((stockCategoryGroup) =>
                      (stockCategoryGroup.rows || []).map((row) => ({
                        ...row,
                        __location: locationGroup.group,
                        __stockBatch: stockBatchGroup.group,
                        __itemCode: itemGroup.group,
                        __category: stockCategoryGroup.group,
                      })),
                    ),
                  ),
                ),
              ...
              .reduce((acc, row) => {
                const key = `${row.__location}-${row.__stockBatch}-${row.__itemCode}-${row.__category}`;
            ...
            return (
              <View
                key={`${group.location}-${group.stockBatch}-${group.itemCode}-${group.category}`}
                wrap
              >
                {/* Group Title */}
                <Text style={styles.groupTitle}>
