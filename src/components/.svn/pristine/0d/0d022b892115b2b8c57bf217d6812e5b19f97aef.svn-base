...
interface MobilePopoverDataTableProps<T> {
  table: TableType<T>;
...
export default function MobilePopoverDataTable<T extends Record<string, any>>({
  table,
  headerFields,
              ...
              <div className="flex items-center">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    if (onRowClick) {
                      onRowClick(row)({ detail: 1 } as any);
                    }
                  }}
                  aria-label="Select row"
                  className="h-5 w-5"
                />
                <span className="ml-1 text-sm">{selectionLabel}</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {table.getAllColumns().map((column, index) => {
              ...
              if (
                ...
                excludeFromGrid.includes(key as keyof T)
              ) {
                return null;
              }
