...
function ReleaseNotePopover() {
  ...
  const visibleReleases = showAllReleases ? release : release?release.slice(0, 2):[];
  ...
  useEffect(() => {
    ...
    fetch(`${ORIGIN}/site/api/site/get-release-note`, {
      method: "GET",
      headers
    })
      .then((res) => res.json())
          ...
          >
            ...
            >
              {visibleReleases.map((release, index) => (
                release && <ReleaseItem key={index} release={release} />
              ))}
            </CardContent>

            {/* Read More / Show Less Button */}
            {release && release.length > 2 && (
